export default {
  title: "Azure Data Fundamentals - 900",
  subtitle:
    "A master course designed for working professionals like you. Make your career soar with Loyal Bytes.",
  heroImage: "/images/course-page/azure-hero-image.png",
  heroBg: "/images/course-page/azure-hero-bg.png",
  courseDetail: {
    duration: "8 Hours",
    certification: "Microsoft Certification",
    batchInfo: "Weekdays batches available",
    level: "100",
    highlights: [
      {
        icon: "user",
        color: "orange",
        heading: "Duration",
        subheading: "8 Hours.",
      },
      {
        icon: "user ",
        color: "orange",
        heading: "Batch Days",
        subheading: "Weekdays",
      },
      {
        icon: "user ",
        color: "orange",
        heading: "Learning mode",
        subheading:
          "Group, One-to-One, Corporate Batches, Online Live Classrooms",
      },
      {
        icon: "user",
        color: "orange",
        heading: "Passing score",
        subheading: "700",
      },
    ],
    bg: "blue",
    displayImage: "/images/course-page/azure-hightlights-image.png",
    description:
      "To master data in the cloud, you need the right foundation—a solid understanding of core data concepts, such as relational data, nonrelational data, big data, and analytics. Plus, familiarity with the roles, tasks, and responsibilities in the world of data and data analytics. Are you there? Certification can help you prove it. The certification offers the foundation you need to build your technical skills to start working with data in the cloud. Mastering the basics can help you jump-start your career and prepare you to dive deeper into other technical opportunities Azure offers. If you’re beginning to work with data in the cloud, this certification is for you. It is recommended to have familiarity with the concepts of relational and nonrelational data and with different types of data workloads, such as transactional or analytical.",
  },
  idealFor:
    "This exam is an opportunity to demonstrate knowledge of core data concepts and related Microsoft Azure data services. Candidates for this exam should have familiarity with DP-900&apos;s self-paced or instructor-led learning material. This exam is intended for candidates beginning to work with data in the cloud. •	Candidates should be familiar with the concepts of relational and non-relational data, and different types of data workloads such as transactional or analytical. Azure Data Fundamentals can be used to prepare for other Azure role-based certifications like Azure Database Administrator Associate or Azure Data Engineer Associate, but it is not a prerequisite for any of them.",

  learningObjectives:
    "Validate your foundational knowledge of core data concepts and how they&apos;re implemented using Azure data services. Reinforce your basics for other Azure role-based certifications, like Azure Database Administrator Associate, Azure Data Engineer Associate, Data Analyst Associate, or Azure Developer Associate, but it’s not a prerequisite for any of them.",
  modules: [
    {
      title: "Module 1:	DESCRIBE CORE DATA CONCEPTS (25–30%)",
      submodules: [
        {
          heading: "9.1.1	DESCRIBE WAYS TO REPRESENT DATA",
          contents: [
            "describe features of structured data",
            "describe features of semi-structured ",
            "describe features of unstructured data ",
          ],
        },
        {
          heading: "9.1.2	IDENTIFY OPTIONS FOR DATA STORAGE",
          contents: [
            "describe common formats for data files ",
            "describe types of databases ",
          ],
        },
        {
          heading: "9.1.3	DESCRIBE COMMON DATA WORKLOADS ",
          contents: [
            "describe features of transactional workloads ",
            "describe features of analytical workloads ",
          ],
        },
        {
          heading: "9.1.3	DESCRIBE COMMON DATA WORKLOADS ",
          contents: [
            "describe features of transactional workloads ",
            "describe features of analytical workloads ",
          ],
        },
        {
          heading:
            "9.1.4	IDENTIFY ROLES AND RESPONSIBILITIES FOR DATA WORKLOADS ",
          contents: [
            "describe responsibilities for database administrators ",
            "describe responsibilities for data engineers ",
            "describe responsibilities for data analysts",
          ],
        },
      ],
    },
    {
      title:
        "Module 2:	IDENTIFY CONSIDERATIONS FOR RELATIONAL DATA ON AZURE (20–25%) ",
      submodules: [
        {
          heading: "9.2.1	DESCRIBE RELATIONAL CONCEPTS ",
          contents: [
            "identify features of relational data ",
            "describe normalization and why it is used ",
            "identify common structured query language (SQL) statements ",
            "identify common database objects ",
          ],
        },
        {
          heading: "9.2.2	DESCRIBE RELATIONAL AZURE DATA SERVICES ",
          contents: [
            "describe the Azure SQL family of products including Azure SQL Database, Azure SQL ",
            "managed Instance, and SQL Server on Azure Virtual Machines ",
            "identify Azure database services for open-source database systems ",
          ],
        },
      ],
    },
    {
      title:
        "Module 3: DESCRIBE CONSIDERATIONS FOR WORKING WITH NON-RELATIONAL DATA ON AZURE (15–20%) ",
      submodules: [
        {
          heading: "9.3.1	DESCRIBE CAPABILITIES OF AZURE STORAGE ",
          contents: [
            "describe Azure Blob storage ",
            "describe Azure File storage ",
            "describe Azure Table storage ",
          ],
        },
        {
          heading:
            "9.3.2	DESCRIBE CAPABILITIES AND FEATURES OF AZURE COSMOS DB ",
          contents: [
            "identify use cases for Azure Cosmos DB ",
            "describe Azure Cosmos DB APIs ",
          ],
        },
      ],
    },
    {
      title: "Module 4: DESCRIBE AN ANALYTICS WORKLOAD ON AZURE (25–30%) ",
      submodules: [
        {
          heading: "9.4.1	DESCRIBE COMMON ELEMENTS OF A MODERN DATA WAREHOUSE ",
          contents: [
            "describe considerations for data ingestion and processing ",
            "describe options for analytical data stores ",
            "describe Azure services for data warehousing, including Azure Synapse Analytics, Azure Databricks, Azure HDInsight, and Azure Data Factory ",
          ],
        },
        {
          heading: "9.4.2	DESCRIBE CONSIDERATION FOR REAL-TIME DATA ANALYTICS ",
          contents: [
            "describe the difference between batch and streaming data ",
            "describe technologies for real-time analytics including Azure Stream Analytics, Azure Synapse Data Explorer, and Spark structured streaming ",
          ],
        },
        {
          heading: "9.4.3	DESCRIBE DATA VISUALIZATION IN MICROSOFT POWER BI ",
          contents: [
            "identify capabilities of Power BI ",
            "describe features of data models in Power BI ",
            "identify appropriate visualizations for data ",
          ],
        },
      ],
    },
  ],
  onCompletion: [
    "The bullets that follow each of the skills measured are intended to illustrate how we are assessing that skill. Related topics may be covered in the exam. ",
    "Most questions cover features that are general availability (GA). The exam may contain questions on Preview features if those features are commonly used.",
  ],
  batches: [
    {
      id: "",
      days: "",
      mode: "",
      time: "",
      duration: "",
    },
    {
      id: "",
      days: "",
      mode: "",
      time: "",
      duration: "",
    },
    {
      id: "",
      days: "",
      mode: "",
      time: "",
      duration: "",
    },
    {
      id: "",
      days: "",
      mode: "",
      time: "",
      duration: "",
    },
    {
      id: "",
      days: "",
      mode: "",
      time: "",
      duration: "",
    },
  ],
};
